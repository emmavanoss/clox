...
static void runtimeError(const char* format, ...) {
  ...
  for (int i = vm.frameCount - 1; i >= 0; i--) {
    ...
    // -1 because ip is sitting on next instruction to be executed
...
InterpretResult interpret(const char* source) {
  ObjFunction* function = compile(source); // returns main script function
  if (function == NULL) return INTERPRET_COMPILE_ERROR;

  push(OBJ_VAL(function)); // store the main script function on the stack
  callValue(OBJ_VAL(function), 0); // set up first callframe
